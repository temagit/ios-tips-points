...
### 低内存闪退
因为低内存闪退与普通内存系统发出来的日志略有不同。

iOS 设备检测到低内存是，虚拟内存系统发出通知请求应用释放压力，这些通知发送到所有正在运行的应用和进程，试图回收一些内存。
如果内存使用依然居高不下，系统将会终止后台线程以缓解内存压力，如果可用内存足够，应用将能继续运行，而不会产生崩溃报告，否则，应用将被系统终止，并且产生低内存崩溃报告。

低内存崩溃日志上没有应用线程的堆栈回溯，相反，上面显示的是以内存页面为单位的各进程内存使用量（在撰写文本的时候，一个内测页的大小是4kb）

被iOS因为释放内存页终止的进程名称后面你会看到jettisoned 字样，如果看到它出现在你的应用名称后面，说明你的应用因使用太多内存而被终止了Automatic Snapshotting

![](./low_memory.png)

当发生低内存闪退时，需要用Instruments检查内存问题。Instrumensts 工具的Leaks和Allocations不能跟踪显存使用情况，必须使用VM Tracker才能查看显存使用情况。VM Tracker默认是关闭的， 打开Instrument，手动选中Automatic Snapshotting 标志或者按下Snapshot Now 按钮
